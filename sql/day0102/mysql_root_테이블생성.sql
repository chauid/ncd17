...
-- table 구조 확인
desc person;
show create table person; -- 좀더 디테일한 구조확인

-- 데이타 추가하기
insert into person (name,age,today,writeday) values ('이영자',23,now(),now());
-- 모든컬럼명 생략하고 순서대로 넣기-num운 시퀀스 컬럼이므로 null 로 줘도 숫자로 들어간다
...
select * from person where name like '%영%'; -- 영을 포함한경우
select * from person where name like '_진%'; -- 두번째 글자가 '진'인사람
